        ...
        <>
          ...
          <section id="features" className="mt-10">
            ...
            <div className="mt-5 grid md:grid-cols-2 gap-4">
              {(t.raw('docs.gettingStarted.features.items') as { title: string; desc: string }[]).map(
                (item) => (
                  <div
                    key={item.title}
                    className="rounded-xl border border-gray-200/70 dark:border-white/10 bg-white/70 dark:bg-black/20 p-5"
                  >
                    <div className="font-heading text-lg font-semibold text-gray-900 dark:text-white mb-2">
                      {item.title}
                    </div>
